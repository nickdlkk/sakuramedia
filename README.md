...
<h2 align="center">插件机制与贡献</h2>

SakuraMediaBE 已提供插件机制，欢迎社区贡献新的插件。插件以独立的 `.zip` 包分发，用户可以在客户端「系统设置 → 插件」中上传、启用、停用、删除插件，并编辑插件的私有 JSON 配置；插件安装、状态或配置变更后，重启容器即可生效。

当前插件机制主要提供以下能力：

- 注册定时任务、手动任务或带参数的任务，并接入统一的任务中心、进度反馈、运行记录和任务触发流程。
