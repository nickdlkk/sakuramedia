...
| **[navigation.md](./navigation.md)** | `AppTabBar` / `AppListHeader`(+`AppListHeaderInfo`) / `AppFilterEntryButton` / `AppMobileFilterDrawerScaffold` | 要横向切分内容(Tab),或做列表顶栏(筛选 / 信息 / 操作,双端同一条) |
| **[data-loading.md](./data-loading.md)** | `AppPullToRefresh` / `AppAdaptiveRefreshScrollView` / `AppPagedLoadMoreFooter` / `AppFilterTotalHeader` / `AppText` / `AppAdaptiveCardGrid<T>` / `MultiSelectStateMixin` / `SelectionCheckBadge` / `AppSelectionToolbar` / `StaggeredLayout*` | 要接列表下拉刷新 / 分页脚 / 总数条 / **卡片自适应网格** / 多选状态与操作条 / 交错布局 / 文字统一样式 |
