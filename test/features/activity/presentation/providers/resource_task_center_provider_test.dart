...
void main() {
  ...
  test('initialize 加载 definitions 并默认选中第一个任务 + 拉取第一页记录', () async {
    ...
    final controller = _ResourceTaskCenterHarness(
  ...
  group('批量重置 (media_thumbnail_generation)', () {
    Future<_ResourceTaskCenterHarness> initMediaTab(
...
class _ResourceTaskCenterHarness {
  _ResourceTaskCenterHarness({required ActivityApi activityApi})
    : _container = ProviderContainer(
        overrides: [activityApiProvider.overrideWithValue(activityApi)],
      ) {
    _subscription = _container.listen<AsyncValue<ResourceTaskCenterState>>(
  ...
  late final ProviderSubscription<AsyncValue<ResourceTaskCenterState>>
  _subscription;

  ResourceTaskCenterState get _state =>
      _container.read(resourceTaskCenterProvider).requireValue;
  ResourceTaskCenter get _notifier =>
      _container.read(resourceTaskCenterProvider.notifier);

  Future<void> initialize() async {
    await _container.read(resourceTaskCenterProvider.future);
  }
  ...
  List<ResourceTaskDefinitionDto> get definitions => _state.definitions;
  String? get activeTaskKey => _state.activeTaskKey;
  ResourceTaskDefinitionDto? get activeDefinition => _state.activeDefinition;
  List<ResourceTaskRecordDto> get activeRecords => _state.activeRecords;
  ...
  ResourceTaskRecordDto? get selectedRecord => _state.selectedRecord;
  bool get selectionMode => _state.selectionMode;
  int get selectedCount => _state.selectedCount;
  bool get isResetting => _state.isResetting;
  bool get supportsBatchReset => _notifier.supportsBatchReset;
  int get visibleFailedCount => _notifier.visibleFailedCount;
  int get visibleFailedTotalCount => _notifier.visibleFailedTotalCount;
  bool get isAllVisibleFailedSelected => _notifier.isAllVisibleFailedSelected;
  bool isRecordSelected(int resourceId) =>
      _notifier.isRecordSelected(resourceId);

  Future<void> selectTaskKey(String taskKey) =>
      _notifier.selectTaskKey(taskKey);
  Future<void> applyFilter(ResourceTaskRecordFilterState next) =>
      _notifier.applyFilter(next);
  Future<void> loadMoreRecords() => _notifier.loadMoreRecords();
  Future<void> refreshRecords() => _notifier.refreshRecords();
  Future<void> refreshDefinitions() => _notifier.refreshDefinitions();
  void openDetail(ResourceTaskRecordDto record) => _notifier.openDetail(record);
  void closeDetail() => _notifier.closeDetail();
  void enterSelectionMode() => _notifier.enterSelectionMode();
  bool toggleRecordSelection(int resourceId) =>
      _notifier.toggleRecordSelection(resourceId);
  void toggleSelectAllVisibleFailed() =>
      _notifier.toggleSelectAllVisibleFailed();
  Future<ResourceTaskActionResultDto?> resetSelectedFailed() =>
      _notifier.resetSelectedFailed();
}
